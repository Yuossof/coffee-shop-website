...
function Footer({ variant = 'home' }) {
  ...
  return (
    <footer className="bg-peach py-16">
      <Container className="grid grid-cols-1 gap-6 pb-12 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
        <div>
          <p className={brandNameClass}>Aura Coffee & Bakes</p>
          <p className={taglineClass}>
            Artisanal moments, daily rituals. A quiet corner for exceptional
            coffee and hand-crafted pastries.
          </p>
          <a
            href="#social"
            aria-label="Social"
            className="mt-6 flex h-10 w-10 items-center justify-center rounded-full bg-peach-2 font-semibold text-heading-brown"
          >
            @
          </a>
        </div>
      ...
      <Container className="flex flex-col items-start justify-between gap-2 border-t border-border-pink-30 pt-8 text-base text-body-brown sm:flex-row sm:items-center">
        <p>© 2024 Aura Coffee & Bakes. Artisanal moments, daily rituals.</p>
      </Container>
    </footer>
  )
}
